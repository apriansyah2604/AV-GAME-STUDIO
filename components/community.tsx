  ...
  return (
    <section id="community" className="relative py-20 lg:py-32 overflow-hidden">
      ...
      <div 
        ...
        style={{
          backgroundImage: `
            linear-gradient(rgba(255, 70, 85, 0.2) 1px, transparent 1px),
            linear-gradient(90deg, rgba(255, 70, 85, 0.2) 1px, transparent 1px)
          `,
      ...
      {/* Ambient Glow */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[#ff4655]/5 rounded-full blur-3xl" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#ff4655]/5 rounded-full blur-3xl" />
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <motion.a
              ...
              whileTap={{ scale: 0.98 }}
              className="inline-flex items-center gap-3 px-8 py-4 rounded-none bg-[#5865F2] hover:bg-[#4752C4] transition-colors group skew-x-[-12deg]"
            >
              <span className="skew-x-[12deg] flex items-center gap-3">
                ...
                <span className="font-black uppercase tracking-normal italic">{t('community.discord_btn')}</span>
            ...
            <div className="mt-8 flex items-center gap-3">
              ...
              <span className="text-sm text-white/60 font-medium">
                <span className="text-emerald-400 font-black uppercase italic">2,847</span> {t('community.online_status')}
              </span>
            </div>
          </motion.div>
          ...
          >
            <div className="relative">
              ...
              <div className="glass-strong rounded-none p-8 border border-white/10">
                {/* Stats Grid */}
                <div className="grid grid-cols-2 gap-6 mb-8">
                  {stats.map((stat: any, index: number) => (
                    <motion.div
                      ...
                      className="text-center p-4 rounded-none bg-[#111827]/50 border border-white/5"
                    >
                      <stat.icon className="w-6 h-6 text-[#ff4655] mx-auto mb-2" />
                      <div className="text-2xl font-black neon-text uppercase tracking-tight">{stat.value}</div>
                      <div className="text-[10px] text-white/50 tracking-normal font-black uppercase italic">{stat.label}</div>
                    </motion.div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {socialLinks.map((social: any, index: number) => (
                    <motion.a
                      ...
                      className="flex items-center justify-between p-3 rounded-none bg-[#111827]/30 border border-white/5 hover:border-[#ff4655]/50 transition-colors group"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-none bg-[#1e293b] flex items-center justify-center skew-x-[-12deg]">
                          <span className="text-lg font-black text-[#ff4655] -skew-x-[-12deg]">{social.name[0]}</span>
                        </div>
                        <span className="font-black text-white/80 text-xs uppercase tracking-normal">{social.name}</span>
                      </div>
                      <div className="flex items-center gap-2 text-[10px] text-white/50 font-black italic">
